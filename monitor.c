...
{
	FILE *max_watches = my_fopen("/proc/sys/fs/inotify/max_user_watches", "r+");
	if (!max_watches)
		return;
	if (!limit)
...
inotify_create_watches(int fd)
{
	...
	max_watches = my_fopen("/proc/sys/fs/inotify/max_user_watches", "r");
	if( max_watches )
	{
		...
		{
			if (my_access("/proc/sys/fs/inotify/max_user_watches", W_OK) == 0)
			{
...
#ifdef _WIN32
		strncpy(path_buf, fullPath, PATH_BUF_SIZE);
...
#endif
{
	...
	if (
#ifdef _WIN32
		(SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_BELOW_NORMAL) == 0)
#else
