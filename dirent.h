...
static DIR *opendir(const char *dirname)
{
	...
	size_t n;
	n = MultiByteToWideChar(CP_UTF8, 0, dirname, -1, wname, PATH_MAX + 1);
	if (n == 0)
		goto exit_failure;
	...
	DIR *dirp, struct dirent *entry, struct dirent **result)
{
	...
	size_t n;
	n = WideCharToMultiByte(CP_UTF8, 0, datap->cFileName, -1, entry->d_name, PATH_MAX + 1, NULL, NULL);
	 ...
	 */
	if (n == 0 && datap->cAlternateFileName[0] != '\0') {
		n = WideCharToMultiByte(CP_UTF8, 0, datap->cAlternateFileName, -1, entry->d_name, PATH_MAX + 1, NULL, NULL);
	}
